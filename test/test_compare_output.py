...
for sheet_name in ['case results', 'component results', 'time results']:
    ...
    expected_df = pd.read_excel('test/test_prefix.xlsx', sheet_name=sheet_name)

    # compare pypsa output to expected output
    comparison_df = output_df.compare(expected_df)
    print('='*40)
    if comparison_df.shape == (0,0):
        print(f'OK, {sheet_name} output == expected')
        print('='*40)
    else:
        print(f'ERROR: {sheet_name} output does not match expected output')
